...
class ModelConfig:
    ...
    def bnn_params(self) -> Dict:
        ...
        return {
            ...
            'use_attention': True,  # Enable attention mechanism switch
            'attention_pos': 0,  # Attention layer position (0: after input layer, 1: after first hidden layer)
...
class SelfAttention(nn.Module):
    """Self-attention module for feature weighting"""
    def __init__(self, feature_dim: int, attention_dim: int = 8):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scores = self.softmax(scores * self.scale)

        # Apply attention weights
        weighted = torch.bmm(scores, V.unsqueeze(1)).squeeze(1)
...
class BNN(nn.Module):
    ...
    def __init__(self, input_dim: int, config: Dict, use_mask: bool = True):
        ...
        # Add attention at input layer
        self.use_attention = config.get('use_attention', False)
        self.attention_pos = config.get('attention_pos', 0)
        self.attention_dim = config.get('attention_dim', 8)
    ...
    def _build_network(self, input_dim: int) -> nn.Sequential:
        ...
        # Add attention after first hidden layer
        if self.use_attention and self.attention_pos == 0:
            layers.append(SelfAttention(prev_dim, self.attention_dim))

        for i, (h_dim, drop_rate) in enumerate(zip(self.hidden_dims, self.dropout_rates)):
            layers.append(nn.Linear(prev_dim, h_dim))

            if self.use_attention and self.attention_pos == 1 and i == 0:
                layers.append(SelfAttention(h_dim, self.attention_dim))
...
class HybridModel:
    ...
    def predict(self, X: pd.DataFrame, n_samples: int = 30) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if np.any(np.isnan(residual_std)) or np.any(np.isinf(residual_std)):
            logger.warning("Residual std contains NaN/inf values")

        # 组合预测
        hybrid_pred = xgb_pred + residual_mean
        ...
        return hybrid_pred, residual_std
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'X_combined' in results and 'y_combined' in results:
            ...
            combined_sto_flags = (results['X_combined'][STO_COLUMN] == 'STO').astype(int).values if STO_COLUMN in results[
