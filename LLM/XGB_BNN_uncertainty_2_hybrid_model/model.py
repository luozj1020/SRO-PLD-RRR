...
def sequence_ranking_loss(bnn_model, residual_stats, sequences, labels, config):
    ...
    # BNN prediction - ensure using correct device
    device = next(bnn_model.parameters()).device
    with torch.no_grad():
        # Move data to BNN model's device
        flat_sequences_device = flat_sequences.to(device)

        # Create default mask for sequence data (all features are valid)
        # Shape should match flat_sequences: (batch_size * seq_len, feature_dim)
        sequence_mask = torch.ones_like(flat_sequences_device, device=device)

        # Pass both features and mask to BNN
        bnn_residuals = bnn_model(flat_sequences_device, sequence_mask).cpu().numpy().reshape(batch_size, seq_len)
        bnn_residuals = bnn_residuals * residual_stats['std'] + residual_stats['mean']

    # Final prediction - ensure device consistency
    final_preds = torch.FloatTensor(bnn_residuals).to(sequences.device)
...
class HybridModel:
    ...
    def _train_bnn(self, X_raw: np.ndarray, X_processed: np.ndarray, y: np.ndarray,
        ...
        self.sigma_xgb_sq = np.var(residuals_clipped)
        if self.sigma_xgb_sq < 1e-8:  # Avoid division by zero
            self.sigma_xgb_sq = 1e-8
    ...
    def predict(self, X: pd.DataFrame, n_samples: int = 30) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if np.any(np.isnan(residual_var)) or np.any(np.isinf(residual_var)):
            ...
            residual_var = np.nan_to_num(residual_var, nan=1e-8)

        # Apply uncertainty-guided weighting
        epsilon = 1e-8

        try:
            if self.sigma_xgb_sq is not None and self.sigma_xgb_sq > epsilon:
                # Weighted fusion based on uncertainties
                w_xgb = 1.0 / (residual_var + epsilon)
                w_bnn = 1.0 / (self.sigma_xgb_sq + epsilon)
                final_pred = (w_xgb * xgb_pred + w_bnn * residual_mean) / (w_xgb + w_bnn)
            else:
                # Fallback to simple addition
                final_pred = xgb_pred + residual_mean

            if np.any(np.isnan(final_pred)) or np.any(np.isinf(final_pred)):
                logger.warning("Final predictions contain NaN/inf values")
                final_pred = np.nan_to_num(final_pred, nan=0.0)
        except Exception as e:
            logger.error(f"Final prediction combination failed: {str(e)}")
            final_pred = xgb_pred

        return final_pred, np.sqrt(residual_var + epsilon)
    ...
    def save_model(self, path: str):
        ...
        joblib.dump({
            ...
            'sigma_xgb_sq': self.sigma_xgb_sq
    ...
    def load_model(cls, path: str) -> 'HybridModel':
        ...
        model.sigma_xgb_sq = params.get('sigma_xgb_sq', None)  # ADD THIS
